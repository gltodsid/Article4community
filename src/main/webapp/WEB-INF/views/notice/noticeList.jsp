			...
			<table id="boardList">
				...
				<c:if test="${!empty vList}">
					<c:forEach var="item" items="${vList}">
						...
						<tr class="prnTr" onclick="read('${item.num}', '${nowPage}')">
					...
					<tr id="listBtnArea">
						<td colspan="2">
							<c:if test="${sessionScope.accLv == '1' or sessionScope.accLv == '2'}">
								<button type="button" id="noticeWrtBtn" class="listBtnStyle">글쓰기</button>
							</c:if>
						</td>
							...
							<form name="searchFrm" class="dFlex" id="searchFrm">
								...
								<div>
									<select name="keyField" id="keyField">
										<option value="title" 
											<c:if test="${keyField eq 'title'}">selected</c:if>>제  목</option>
										<option value="writer" 
											<c:if test="${keyField eq 'writer'}">selected</c:if>>이  름</option>
